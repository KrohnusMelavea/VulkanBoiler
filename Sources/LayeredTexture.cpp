...
namespace API_NAME {
	...
	VkResult LayeredTexture::create(VkDevice const logical_device, VkPhysicalDevice const physical_device, VkQueue const graphics_queue, VkCommandPool const command_pool, f32 const anistropy) {
		...
		/* Debug Check for image validity */ {
#ifdef _DEBUG
		...
		MappedBuffer image_buffer(logical_device, physical_device);
		/* Create and Populate Buffer */ {
			result = image_buffer.create(valid_image_count * base_image.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
#ifdef _DEBUG
...
#endif
			auto mapped = reinterpret_cast<u08*>(image_buffer.mapped_memory());
		...
		/* Create Image and Copy Buffer into it */ {
